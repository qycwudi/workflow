...
func (m *defaultApiModel) Insert(ctx context.Context, data *Api) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.WorkspaceId, data.ApiId, data.ApiName, data.ApiDesc, data.ApiDoc, data.Tag, data.Dsl, data.HistoryId, data.Status, data.CreateTime, data.UpdateTime)
	return ret, err
}
...
func (m *defaultApiModel) Update(ctx context.Context, newData *Api) error {
	...
	_, err := m.conn.ExecCtx(ctx, query, newData.WorkspaceId, newData.ApiId, newData.ApiName, newData.ApiDesc, newData.ApiDoc, newData.Tag, newData.Dsl, newData.HistoryId, newData.Status, newData.CreateTime, newData.UpdateTime, newData.Id)
	return err
}
