...
func (m *defaultUsersModel) Insert(ctx context.Context, data *Users) (sql.Result, error) {
	...
	ret, err := m.conn.ExecCtx(ctx, query, data.Username, data.Password, data.Salt, data.RealName, data.Phone, data.Email, data.Status, data.CreatedAt, data.UpdatedAt, data.Uid)
	return ret, err
}

func (m *defaultUsersModel) Update(ctx context.Context, newData *Users) error {
	query := fmt.Sprintf("update %s set %s where `id` = ?", m.table, usersRowsWithPlaceHolder)
	_, err := m.conn.ExecCtx(ctx, query, newData.Username, newData.Password, newData.Salt, newData.RealName, newData.Phone, newData.Email, newData.Status, newData.CreatedAt, newData.UpdatedAt, newData.Uid, newData.Id)
	return err
}
