...
type (
	...
	ApiSecretKeyModel interface {
		...
		FindOneByApiIdAndSecretKey(ctx context.Context, apiId, secretKey string) (*ApiSecretKey, error)
...
func (m *customApiSecretKeyModel) FindOneByApiIdAndSecretKey(ctx context.Context, apiId, secretKey string) (*ApiSecretKey, error) {
	query := fmt.Sprintf("select %s from %s where api_id = ? and secret_key = ? and is_deleted = 0 limit 1", apiSecretKeyRows, m.table)
	var resp ApiSecretKey
	err := m.conn.QueryRowCtx(ctx, &resp, query, apiId, secretKey)
	switch err {
	case nil:
		return &resp, nil
	case sqlc.ErrNotFound:
